...
- [x] **Phase 5: Multi-Uploader & Phân Quyền Phát Nhạc (Stream Access Control: Only Me • Whitelist • Public) (P1)** *(Hoàn thành 06/09/2026)*:
  ...
  - [x] **Desktop Selective Sync & Giao Diện Quản Trị (`apps/desktop`)**:
    ...
    - [x] **HTTP Request Logger Middleware (`apps/server/src/app.ts`)**: Tích hợp middleware ghi log thời gian thực cho toàn bộ HTTP request (`[HTTP] HH:mm:ss {METHOD} {URL} {STATUS} ({DURATION}ms) [user: {username}] - {IP}`). Hiển thị trực tiếp trên terminal server, PM2 log stream và `pm2 monit` phục vụ giám sát và debug đa người dùng.
  - [x] **Kiểm thử toàn diện & Zero TypeScript Errors**:
    - 20/20 server tests Green (16 tests `server.test.ts`, 4 suites `ServerMultiUploader.test.ts`).
