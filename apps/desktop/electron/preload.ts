...
export interface ElectronAPI {
  ...
  checkDuplicate: (title: string, artist: string, url?: string, id?: string) => Promise<DuplicateCheckResult>;
...
const electronAPI: ElectronAPI = {
  ...
  updatePlaylist: (playlist: Partial<Playlist>) => ipcRenderer.invoke('library:updatePlaylist', playlist),
  ...
  savePlaylistsData: (playlists: Record<string, Playlist>) => ipcRenderer.invoke('storage:savePlaylists', playlists),
  ...
  writeAudioMetadata: (filePath: string, metadata: Partial<Song>) => ipcRenderer.invoke('write-audio-metadata', filePath, metadata),
  onDownloadProgress: (callback: (data: { url: string; percent: number }) => void) => {
    const listener = (_event: IpcRendererEvent, data: { url: string; percent: number }) => callback(data);
